...
def cdf(h, v):
    ...
    for i in range(0, nrows):
        ...
        for j in range(0, ncols):
            ...
            # if len(flat_t) != 0:      
            # vfunc = np.vectorize(N, otypes=[np.float64])
            # # code.interact(local=dict(locals(), **globals()))

            # lower_t = np.sum(vfunc((np.repeat(target_h, len(flat_t))- flat_t)/ 0.1))
            # lower_b = np.sum(vfunc((np.repeat(target_h, len(flat_b))- flat_b)/ 0.1))
            # lower_l = np.sum(vfunc((np.repeat(target_v, len(flat_l))- flat_l)/ 0.1))
            # lower_r = np.sum(vfunc((np.repeat(target_v, len(flat_r))- flat_r)/ 0.1))
                
                # print('%.4f %.4f %.4f %.4f' % (lower_t - lower_t2, lower_b - lower_b2, lower_l - lower_l2, lower_r - lower_r2))
...
def segment(filename):
    if isinstance(filename, str):
        ...
        assert(False), "Unknown type %s" % str(type(img))
...
def cormier2_main(img):
    s_list, pic = segment(img)
    ...
    # out_obj = dict(height=pic.shape[0], width=pic.shape[1], segmentations=dict(segmentations=polygon_list))
    return polygon_list
...
def cormier1_main():
    ...
    try:
        outfile = open(args.output_directory + "/cormier_2.json", 'w')
