...
class AdblockRule(object):
	...
	* https://adblockplus.org/en/filters
	...
	BINARY_OPTIONS = [
		...
		"donottrack",
		"websocket"	]
	OPTIONS_SPLIT_PAT = ',(?=~?(?:%s))' % ('|'.join(BINARY_OPTIONS + ["domain", 'redirect', 'replace', 'cookie', 'removeheader', 'removeparam', 'scriptlet', 'empty', 'hls', 'jsonprune', 'header']))
	...
	__slots__ = ['raw_rule_text', 'is_comment', 'is_html_rule', 'is_exception',
				 ...
				 'regex', 'regex_re', 'option_to_name_extension_re']

	def __init__(self, rule_text, enable_domain=True):
		...
		if not self.is_comment and '$' in rule_text:
			...
			self.raw_options = []
			self.options = {}
		if not enable_domain and 'domain' in self.options:
			self.options.pop('domain')

		# code.interact('parse_option3', local=dict(locals(), **globals()))
		...
		# ad block extensions have webrequest.resourcetype filled by the browser to indicate the type of the resource, 
		# we have to guess the resource type from the URL
		option_to_name_extension = {'image': ['png', 'jpg', 'gif', 'webp', 'tif', 'tiff', 'bmp', 'eps', 'raw'],\
			 'script': ['js'], 'stylesheet': ['css']}
		self.option_to_name_extension_re = {k: [re.compile('\.%s[^\w]?' % v ) for v in values] for k, values in option_to_name_extension.items()}

	def __eq__(self, other):
		return self.raw_rule_text == other.raw_rule_text

	def __hash__(self):
		return hash(self.raw_rule_text)
	...
	def check_unsupported_rule(self):
		unsupported = [
			"object",
			# "xmlhttprequest",
			"object-subrequest",
	...
	def param_match(self, url, target_params): # params assumed to be csv 
		"""
		"""
		...
		if not params:
			return False 
		elif params and isinstance(target_params, bool): # to match all params
			return True

		for target in target_params.split(','):
			if target in params:
				return True
		return False 
	...
	def match_url(self, url, options=None):
		...
		"""
		if self.check_unsupported_rule():
			return False
		...
		for optname in self.options:
			...
			if optname == 'domain' and 'domain' in options and options['domain']:
				if not self._domain_matches(options['domain']):
					return False
			elif optname in self.option_to_name_extension_re:
				extensions_re = self.option_to_name_extension_re[optname]
				target = self.options[optname] # this is the target parsed from rule, e.g. ~script is False, image is True

				# if rule requires image, then at least a single image extension must be in the url 
				# if rule requires no script (i.e. target == False), then any('.js' in url) must alse be False 
				if any([re.search(ext_re, url) for ext_re in extensions_re]) != target:
					return False
			elif optname == 'third-party' or optname == '3p' :
				if not ('domain' in options and options['domain']):
					continue

				url_parsed = extract(url)
				top_domain_parsed = extract(options['domain'])
				
				# LHS: (url is first party) RHS: rule applies third party 
				if (url_parsed.domain == top_domain_parsed.domain and url_parsed.suffix == top_domain_parsed.suffix) == self.options[optname]:
					return False		
			# if options[optname] != self.options[optname]:
			# 	return False

		return self._url_matches(url) and ('removeparam' not in self.options or self.param_match(url, self.options['removeparam']))
	...
	def _url_matches(self, url):
		...
		res = bool(self.regex_re.search(url))
		# print('\tvaliding using %s: %s' % (self.regex_re, str(res)))
		return res 
	...
	def _split_options(cls, options_text):
		ret = cls.OPTIONS_SPLIT_RE.split(options_text)
		return ret
	...
	def _parse_option(cls, text):
		...
		for keyword in ['redirect', 'replace', 'cookie', 'removeheader', 'removeparam', 'scriptlet', 'empty', 'hls', 'jsonprune', 'header']:
			if text.startswith(keyword+'='):
				# code.interact('parse_option', local=dict(locals(), **globals()))
				return (keyword, text[len(keyword)+1:]) # +1 for the equal sign
		# code.interact('parse_option2', local=dict(locals(), **globals()))
...
class AdblockRules(object):
	...
	def __init__(self, rules, supported_options=None, skip_unsupported_rules=False,
				 use_re2='auto', max_mem=1*1024*1024*1024, rule_cls=AdblockRule, enable_domain=True):
		...
		self.rules = [
			r for r in (
				r if isinstance(r, rule_cls) else rule_cls(r, enable_domain=self.enable_domain)
				for r in rules
			)
	...
	def should_block_req(self, req, options):
		pass

	def should_block(self, url, options=None):
		# TODO: group rules with similar options and match them in bigger steps
		options = options or {}
		# print("Checking whitelist:", self.whitelist_require_domain)
		if self._is_whitelisted(url, options):
			return False
		# print("Checking blacklist", self.blacklist_require_domain)
	...
	def flip_rule(self, domain, rule, inserting_to):
		if domain in inserting_to:
			inserting_to[domain].append(rule)
		else:
			inserting_to[domain] = [rule]
		
	def flip_domain_rules(self, rule_obj, top_domain, white_to_black=0):
		if white_to_black:
			finding_from = self.whitelist_require_domain
			inserting_to = self.blacklist_require_domain
		else:
			finding_from = self.blacklist_require_domain
			inserting_to = self.whitelist_require_domain

		for k, rules in finding_from.items(): 
			if k != top_domain:
				continue

			for r in rules:
				if r == rule_obj:
					self.flip_rule(k, r, inserting_to)
					finding_from[k].remove(r)
					assert(r in inserting_to), inserting_to
					assert(r not in finding_from), finding_from

					# import code
					# code.interact("flip_exception_rule", local=dict(locals(), **globals()))
					return 
	
	def flip_opt_rules(self, rule_obj, white_to_black):
		if white_to_black:
			finding_from = self.whitelist_with_options
			inserting_to = self.blacklist_with_options
		else:
			finding_from = self.blacklist_with_options
			inserting_to = self.whitelist_with_options
		
		assert(rule_obj in finding_from)
		inserting_to.append(rule_obj)
		finding_from.remove(rule_obj)

		assert(rule_obj in inserting_to), inserting_to
		assert(rule_obj not in finding_from), finding_from


	def flip_basic_rule(self, white_to_black):
		if white_to_black:
			assert(self.blacklist_re is None)
		else: # black to white
			assert(self.whitelist is None)

		self.blacklist_re, self.whitelist_re = self.whitelist_re, self.blacklist_re
			
		
		
	def flip_exception_rule(self, rule_obj, top_domain, white_to_black=0):
		# if 'domain' in rule_obj.options:
		# 	self.flip_domain_rules(rule_obj, top_domain, white_to_black)
		# elif rule_obj.options:
		if rule_obj in self.blacklist_with_options or rule_obj in self.whitelist_with_options:
			self.flip_opt_rules(rule_obj, white_to_black)
		else:
			self.flip_basic_rule(white_to_black)
		

	def block_exception(self, rule_obj):
		for k, rules in self.whitelist_require_domain:
			for r in rules:
				if r == rule_obj:
					if self.last_r:
						self.last_r.enable()
	...
	def _matches(self, url, options,
		...
		# print("Rules to match:%s options:%s " % (str([(rule, rule.matching_supported(options)) for rule in before]), str(options)))
		# import code
...
def _combined_regex(regexes, flags=re.IGNORECASE, use_re2=False, max_mem=None):
	...
	* in case of "DFA out of memory" errors use ``max_mem`` argument
	...
	if use_re2:
		import re2
		options = re2.Options()
		setattr(options, 'max_mem', max_mem)
		setattr(options, 'case_sensitive', False)
